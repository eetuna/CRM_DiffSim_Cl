"""
FULLSTATE MPC controller smoke test (end-to-end).
"""
...
def test_controller_mpc_fullstate_smoke():
    np.random.seed(0)
    n_act = 1
    dt = 0.01
    horizon = 6
    ...
    x_current, p_tip_0 = make_initial_state(params_dict, n_act)
    ...
    for step in range(num_steps):
        ...
        assert u_mpc.shape == (3 * n_act,)
        assert np.all(np.isfinite(u_mpc)), "u_mpc contains NaN/Inf"
        assert np.all(np.isfinite(info['cost_history'])), "cost_history contains NaN/Inf"
        ...
        assert np.linalg.norm(x_next - x_current) > 0.0, "State did not advance"
        x_current = x_next
